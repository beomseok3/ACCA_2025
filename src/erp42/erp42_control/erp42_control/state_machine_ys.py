...
class GetOdometry:
    ...
    def callback_pcl(self, msg):
        ...
        _, _, self.pcl_yaw = euler_from_quaternion(
            [
                msg.pose.pose.orientation.x,
                msg.pose.pose.orientation.y,
                msg.pose.pose.orientation.z,
    ...
    def callback_gps(self, msg):
        if (
            msg.position_covariance[0] < self.covariance_threshold
            and msg.position_covariance[4] < self.covariance_threshold
        ):
            print("kalman")
        ...
        if self.state.value[:-2] == "driving":
            if self.odometry.x != 0.0:  # 10.03 수정
                ...
                adapted_speed = self.ss.adaptSpeed(
                    target_speed, hdr, ctr, min_value=8, max_value=15
                )  # 에러(hdr, ctr) 기반 목표 속력 조정
                speed = self.pid.PIDControl(
                    self.odometry.v * 3.6, adapted_speed, min=8, max=15
                )  # speed 조정 (PI control)
            ...
            if self.odometry.x != 0.0:  # 10.03 수정
                ...
                target_speed = self.set_target_speed()
                adapted_speed = self.ss.adaptSpeed(
                    target_speed, hdr, ctr, min_value=7, max_value=12
                )  # 에러(hdr, ctr) 기반 목표 속력 조정
                speed = self.pid.PIDControl(
            ...
            if self.k < 1:
                try:
                    set_param(
                        "bs_cropbox_filter", "detection_area", "[0.,20.,-2.5,2.5]"
                    )
...
def main():
    ...
    node.declare_parameter("file_name", "mpc_test.db")
