...
class GetOdometry:
    def __init__(self, node, odom_topic, db_mpc):
        self.x = 0.0  # m
        self.y = 0.0  # m
        self.yaw = 0.0  # rad
        self.v = 0.0  # m/s
    ...
    def callback(self, msg):
        ...
        self.steer, self.speed = self.mpc.pose_callback(msg)
...
class StateMachine:
    ...
    def update_cmd_msg(self):
        ...
        # print(self.idx)
        # steer, speed_output = self.mpc.pose_callback(
        #             self.odometry.pose
        #         )
        ...
        if self.state.value[:-2] == "driving" or self.state.value[:-2] == "curve":
            ...
            if self.odometry.x != 0.0:  # 10.03 수정
                ...
                mspeed = self.odometry.speed * 3.6  # kph
                ...
                msg.speed = int(max(0, min(250, mspeed *  10)))
                msg.steer = int(self.odometry.steer * 180.0 / m.pi * -1)
                msg.gear = 2
                msg.brake = int(brake)
            else:
                pass
...
def main():
    ...
    # Declare Params
    node.declare_parameter("file_name", "/bunsudae/bunsudae_0830_v_3" ".db")
    node.declare_parameter("file_name_mpc", "/bunsudae/mpc_bunsudae_0830_v_3" ".db")
    ...
    odometry = GetOdometry(node, odom_topic, db_mpc)
