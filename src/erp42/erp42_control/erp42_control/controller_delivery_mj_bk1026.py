...
class Delivery:
    ...
    def __init__(self, node: Node):
        ...
        # ── 제어 파라미터 ────────────────────────────────────────────────
        self.v_search = 6.0           # 기본 추종 속도 TODO 속도 낮추기 5~6?
        self.v_fast = 12.0             # 재출발 후 빠른 추종 속도
        self.max_steer_deg = 28.0     # 조향 제한(deg)
        self.stop_radius = .0       # [m] sign까지 거리 임계값
        self.stop_hold_sec = 3.0      # [s] E-stop 유지 시간

        # 경로 완료 판정
        self.finish_radius = 1.5      # [m] 마지막 점 반경
    ...
    def control_delivery(self, odometry, abs_var, path):
        ...
        if self.find_sign and (self.place_x is not None) and (self.place_y is not None):
            sign_dist = m.hypot(self.place_x - self.x, self.place_y - self.y)
            print(sign_dist, self.abs_var, self.place_x, self.place_y)

            # 반경 내 진입 시, 약간의 카운트 지연 후 E-stop 래치
            if sign_dist <= self.stop_radius:
                if self.count >= 30:
