...
class PID:
    ...
    def PIDControl(self, speed, desired_value, flag = False):
        ...
        return  int(np.clip(self.speed, int(desired_value-2), int(desired_value+2)))
...
class Obstacle:
        ...
        if len(self.num2_obs) > 1:
            for obs_x, obs_y in self.num2_obs:
                ...
                if len(self.num1_obs) < 1:
                    ...
                    if obs_length <= 3.8:  # 3m -> 2m -> 2.5로 변경 (1012)
                        print("긴급회피")
                        self.to_num = 1
    ...
    def control_obstacle(self, odometry, path):
        ...
        if len(self.local_x) != 0:
            ...
            target_speed = 12 if self.state == "dynamic" else 6
