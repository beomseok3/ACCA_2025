...
from erp42_msgs.msg import AckermannDriveStamped
...
# ---------------------------------------------------
    ...
    Qk: list = field(
        default_factory=lambda: np.diag([8.0, 8.0, 50, 24.0])  # levine sim
        # default_factory=lambda: np.diag([50., 50., 5.5, 13.0])
    )  # state error cost matrix, for the the next (T) prediction time steps [x, y, delta, v, yaw, yaw-rate, beta]
    Qfk: list = field(
        default_factory=lambda: np.diag([12.0,12.0, 50,30.0])  # levine sim
        # default_factory=lambda: np.diag([50., 50., 5.5, 13.0])
    )  # final state error matrix, penalty  for the final state constraints: [x, y, delta, v, yaw, yaw-rate, beta]
    # ---------------------------------------------------
