...
class DB:
    ...
    def write_db_Path_con(
        self, data, path_id: str = "A1A2", default_speed: float = 5.0
    ):
        """
        Path 테이블에 (x, y, yaw, speed) 데이터를 이어서 삽입한다.
        - data       : [(x, y, yaw), ...] 형태
        - path_id    : 동일 경로를 구분할 식별자
        - speed      : 모든 점에 동일 속도(혹은 원하는 값) 부여
        """

        # 1) 지금 테이블에 들어 있는 가장 큰 idx를 가져온다. (없으면 None → -1)
        self.__cur.execute(
            "SELECT COALESCE(MAX(idx), -1) FROM Path WHERE path_id = ?", (path_id,)
        )
        start_idx = self.__cur.fetchone()[0] + 1  # 다음에 쓸 첫 idx

        # 2) 삽입할 레코드 목록 준비
        rows = [
            (path_id, start_idx + i, x, y, yaw, default_speed)
            for i, (x, y, yaw) in enumerate(data)
        ]

        # 3) executemany 로 한 번에 삽입
